...
export const SentMail = () => {
  ...
  const handleClick = (idx)=>{
    navigate(`/dashboard/message/sentMessage?${idx}`)
  }
  return (
    <>
      {
        ...
        appStatus === "connected" && (
          <>
            <div className="px-4 sm:px-6 lg:px-8 py-8">
              <h1 className="text-3xl teu text-slate-800 font-bold">Sent ✨</h1>
              {SentEmails.length > 0 ? (<div className="w-full mb-8 py-4 overflow-hidden rounded-lg shadow-xs">
                <div className="w-full overflow-x-auto">
                  <table className="w-full whitespace-no-wrap">
                    ...
                    >
                      {
                        [...SentEmails].reverse().map((data, index) => {
                          return (
                            <tr className="text-gray-700 cursor-pointer" onClick={()=>{
                              handleClick(index)
                            }} key={index}>
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center text-sm">
                                  ...
                                  <div>
                                    <p className="font-semibold">To: {shortenAddress(data.receiver)}</p>
                                  </div>
                                </div>
                              </td>
              ...
              </div>): (<div>There are no message in the SentMail.</div>)}
            </div>
          </>)
      }
    </>
