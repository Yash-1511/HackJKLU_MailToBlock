...
export const Spammail = () => {
  ...
  const handleClick = (idx)=>{
    navigate(`/dashboard/message/spamMessage?${idx}`)
  }
  return (
    <>
      {
      ...
      }
      {
        appStatus === "connected" && (
          ...
          <div className="px-4 sm:px-6 lg:px-8 py-8">
              <h1 className="text-3xl teu text-slate-800 font-bold">Spams ✨</h1>

              {SpamMails.length > 0 ? ( <div className="w-full mb-8 py-4 overflow-hidden rounded-lg shadow-xs">
                <div className="w-full overflow-x-auto">
                  <table className="w-full whitespace-no-wrap">
                    ...
                    >
                      ...
                      {
                        [...SpamMails].reverse().map((data, idx) => {
              ...
              </div>) : (<div>There are no message in the Spammail.</div>)}
            </div>
          </>)
      }
    </>
